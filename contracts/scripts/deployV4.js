...
async function main() {
  ...
  // 2. Deploy PredictionMarketV4 (升级版：支持 CPMM + LMSR 双算法)
  ...
  // 3. Deploy CreatorMarketV3 (升级版：支持多曲线类型)
  ...
  const addresses = {
    ...
    features: {
      predictionMarket: {
        ...
        description: "支持 CPMM 和 LMSR 双定价算法"
      },
      creatorMarket: {
        version: 3,
        curveTypes: ["LINEAR", "EXPONENTIAL", "SIGMOID"],
        description: "支持线性、指数、Sigmoid 三种联合曲线"
      }
    }
  };
...
`;
  ...
  try {
    ...
    console.log("✅ Updated frontend/src/constants/config.ts");
  } catch (error) {
  ...
  let existingPrivateKey = "你的私钥";
  let existingRapidApiKey = "你的RapidAPI密钥";
  ...
  if (fs.existsSync(oracleEnvPath)) {
    ...
    if (privateKeyMatch && privateKeyMatch[1] && !privateKeyMatch[1].includes('你的')) {
      existingPrivateKey = privateKeyMatch[1];
    }
    if (rapidApiMatch && rapidApiMatch[1] && !rapidApiMatch[1].includes('你的')) {
...
`;
  ...
  try {
    ...
    console.log("✅ Updated oracle-node/.env");
    
    fs.writeFileSync(path.join(oracleDir, ".env.example"), envContent);
    console.log("✅ Updated oracle-node/.env.example");
  } catch (error) {
    console.log("⚠️ Could not update oracle .env:", error.message);
  }
  ...
  console.log("\n🆕 New Features in V4:");
  console.log("   📊 PredictionMarket: CPMM + LMSR 双定价算法");
  console.log("   📈 CreatorMarket: Linear/Exponential/Sigmoid 多曲线");
  console.log("=".repeat(60));

  // 7. 验证提示
  console.log("\n📝 Next Steps:");
  console.log("   1. 更新前端 hooks 以支持新的合约 ABI");
  console.log("   2. 如果在测试网，可以运行验证脚本:");
  ...
  console.log("   3. 测试新功能:");
  console.log("      - 创建 LMSR 算法的预测市场");
  console.log("      - 创建使用指数/Sigmoid 曲线的创作者");
}
